...
객체 지향 프로그래밍(OOP) 언어는 크게 2가지로 나뉜다. 첫번째는 우리가 자주 보고 들었던 `클래스 기반 언어`(예를 들어 자바, C++ 등), 두번째는 잘 듣지 못했던(?)`프로토타입 기반 언어`로서 우리가 지금 사용하는 자바스크립트가 이에 해당한다. 우리는 자바를 사용할 때는 클래스를 기반으로 코딩하고 최대한 객체지향적으로 만들기 위해서 노력한다. ( 객체지향이 무엇인지 잘 모름에도 말이다. 😭 )
...
자바에서 클래스는 객체를 만들기 위한 설계도라고 볼 수 있다. 속성과

마찬가지로 자바스크립트의 프로토타입도 객체를 만들기 위한 설계도이다. 새로운 객체가 만들어질 때, 프로토타입 기반으로 만들어진다.

객체지향적인 구현되어 있는 자바스크립트를 이해하기 위해선 반드시 프로토타입이 무엇인지 알아야 할 것이다.
