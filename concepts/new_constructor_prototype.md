...
위의 코드에서 각각의 함수가 호출될 때 this의 값은 무엇이고 함수가 호출될 때의 반환값은 무엇일지 생각해보자.

첫번째(1)는 `일반 함수의 호출`이다. 이 때의 this는 (브라우저인 경우)`window`를 나타낸다. 또한 함수의 반환값으로 출력되는 값은 `undefined` 이다. 일반 함수의 경우 명시적으로 `return` 키워드를 사용하지 않으면 기본적으로 `undefined`를 반환한다.

두번째는 `생성자 함수의 호출`이다. 이 때의 this는 위와 다르게 `User의 인스턴스, 새로운 객체`를 나타낸다. 그래서 그 객체 안에 nickname과 email을 추가한다. 또한 new 연산자로 함수를 호출하게 되면 아래와 같은 이미지 속의 객체를 얻을 수 있다. 이것은 `new 연산자`에 의해서 생성자 함수와 일반 함수의 호출이 다르게 작동하기 때문에 나타난 결과이다.
