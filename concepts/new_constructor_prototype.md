...
여기서 약간 의문인 부분이 있다. 위의 말을 이해해보면 생성된 객체에서 자신의 생성자 함수를 알기위해서 프로토타입 객체에 접근할 수 있다라고 말하고 있다. 어떻게 그럴 수 있지? 이 부분이 바로 `프로토타입 링크(Prototype Link)`에 해당하는 것이다. 즉, 지금까지 지속적으로 나왔지만 알지 못했던 그 `__proto__` 이다. 이제부터 이 부분을 알아보자.
